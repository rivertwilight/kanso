...
![](/image/post/22cb60fd-4a96-803d-8943-cf3b4584973d_asynccode)
...
-   Layer：垂直来看，地图可以拆分为一个个 Layer，例如水体是一个 Layer，道路是一个 Layer，文字是一个 layer。缩放层级从 0（全球视图）到 20+（街道级细节）。每增加一级缩放，Tile 数量增加 4 倍（横纵各翻倍）。
...
由于默认的配置不支持等高线，然而《荒野大镖客 2》中的地图是有等高线的，所以我们需要**添加一个额外的数据源**，来实现等高线层。这里可以直接使用 maptiler 的数据源：
...
![](/image/post/22cb60fd-4a96-8083-939a-cccf9eb92936_asynccode)

...
// Launch screen timing constants (in milliseconds)
const LAUNCH_SCREEN_FADE_OUT_DURATION = 300;
const LAUNCH_SCREEN_COMPLETE_DELAY = 1500;
const PROGRESS_BAR_MIN_DELAY = 50;
const PROGRESS_BAR_MAX_DELAY = 150;
const PROGRESS_BAR_MIN_INCREMENT = 5;
const PROGRESS_BAR_MAX_INCREMENT = 20;
...
const KindleBezel: React.FC<KindleBezelProps> = ({
	...
	// Start as false so SSR HTML shows content (good for LCP, crawlers, no-JS)
	...
	useEffect(() => {
		if (showLaunchScreen && progress < 100) {
			const randomDelay =
				Math.random() *
					(PROGRESS_BAR_MAX_DELAY - PROGRESS_BAR_MIN_DELAY) +
				PROGRESS_BAR_MIN_DELAY;
			const randomIncrement =
				Math.random() *
					(PROGRESS_BAR_MAX_INCREMENT - PROGRESS_BAR_MIN_INCREMENT) +
				PROGRESS_BAR_MIN_INCREMENT;
			...
			const fadeTimer = setTimeout(() => {
				setIsFadingOut(true);
			}, LAUNCH_SCREEN_COMPLETE_DELAY);

			return () => clearTimeout(fadeTimer);
		}
	}, [showLaunchScreen, progress, isFadingOut]);

	// Hide launch screen after fade out completes
	useEffect(() => {
		if (isFadingOut) {
			const timer = setTimeout(() => {
				setShowLaunchScreen(false);
				sessionStorage.setItem("kindle-launch-shown", "true");
			}, LAUNCH_SCREEN_FADE_OUT_DURATION);
			return () => clearTimeout(timer);
		}
	}, [isFadingOut]);
	...
	return (
		...
		>
			...
			>
				...
				/>
				<DialogPortalContext.Provider
					value={{ portalRef: mobilePortalRef }}
				>
					{children}
				</DialogPortalContext.Provider>
			</div>
